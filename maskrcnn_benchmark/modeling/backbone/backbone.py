...
def build_efficientnet_backbone(cfg):
    body = EfficientNet.from_pretrained(cfg.MODEL.EFFICIENTNETS.NETWORK)
    model = nn.Sequential(OrderedDict([("body", body)]))
    return model
...
def build_efficientnet_fpn_p3p7_backbone(cfg):
    body = EfficientNet.from_pretrained(cfg.MODEL.EFFICIENTNETS.NETWORK)
    out_channels = cfg.MODEL.BACKBONE.OUT_CHANNELS
    fpn = fpn_module.FPN(
        in_channels_list=body.get_list_features()[-5:],
...
def build_efficientnet_fpn_p2p7_backbone(cfg):
    body = EfficientNet.from_pretrained(cfg.MODEL.EFFICIENTNETS.NETWORK)
    out_channels = cfg.MODEL.BACKBONE.OUT_CHANNELS
    fpn = fpn_module.FPN(
        in_channels_list=body.get_list_features()[-5:],
...
_BACKBONES = {"resnet": build_resnet_backbone,
              "resnet-fpn": build_resnet_fpn_backbone,
              "resnet-fpn-retina": build_resnet_fpn_p3p7_backbone,
              "efficientnet": build_efficientnet_backbone,
              "efficientnet-fpn": build_efficientnet_fpn_backbone,
              "efficientnet-fpn-retina": build_efficientnet_fpn_p3p7_backbone,
             }
...
def build_backbone(cfg):
    ...
    elif cfg.MODEL.BACKBONE.CONV_BODY.startswith("E-"):
        if cfg.MODEL.BACKBONE.CONV_BODY.endswith("-FPN"):
            if cfg.RETINANET.RETINANET_ON:
                if cfg.RETINANET.BACKBONE == "p3p7":
                    return build_efficientnet_fpn_p3p7_backbone(cfg)
                elif cfg.RETINANET.BACKBONE == "p2p7":
                    return build_efficientnet_fpn_p2p7_backbone(cfg)
                else:
                    raise Exception("Wrong Setting {}:{}".format(
                        'cfg.RETINANET.BACKBONE', cfg.RETINANET.BACKBBACKBONE))
            else:
                return build_efficientnet_fpn_backbone(cfg)
        return build_efficientnet_backbone(cfg)
