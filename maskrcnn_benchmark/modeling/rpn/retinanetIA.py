...
from .free_anchor_loss_IA import make_free_anchor_loss_evaluatorIA
...
class RetinaNetHead(torch.nn.Module):
    ...
    def forward(self, x, mask):
        ...
        bbox_reg = []
        if mask is not None:
            for (feature, mask_) in zip(x, mask):
                if self.training:
                    if self.cfg.FREEANCHOR.MASK_TYPE == 0:
                        feature = (3*feature * mask_ + feature) / 4.0
                    elif self.cfg.FREEANCHOR.MASK_TYPE == 1:
...
class RetinaNetModule(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        loss_evaluator = make_free_anchor_loss_evaluatorIA(cfg, box_coder) if cfg.FREEANCHOR.FREEANCHOR_ON \
    ...
    def forward(self, images, features, mask=None, gflag=None, targets=None):
        """
        Arguments:
        ...
        """
        box_cls, box_regression = self.head(features, mask)
