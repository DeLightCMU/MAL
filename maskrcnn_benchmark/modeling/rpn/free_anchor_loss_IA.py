...
class FreeAnchorLossComputation(object):
    ...
    def __call__(self, anchors, box_cls, box_regression, targets):
        ...
        for img, (anchors_, targets_, cls_prob_, box_regression_) in enumerate(
            ...
            pred_info = {}
            pred_info['box_cls'] = cls_prob_
            pred_info['box_regression'] = box_regression_
            pred_info['matched'] = matched
            pred_info['matched_cls_prob'] = matched_cls_prob
            pred_info['matched_object_targets'] = matched_object_targets

            batch_pred_info.append(pred_info)
...
def positive_bag_loss_mean(logits, *args, **kwargs):
    # bag_prob = mean(logits)
    bag_prob = logits.mean(*args, **kwargs)
    return F.binary_cross_entropy(bag_prob, torch.ones_like(bag_prob), reduction='none')
...
def make_free_anchor_loss_evaluatorIA(cfg, box_coder):
